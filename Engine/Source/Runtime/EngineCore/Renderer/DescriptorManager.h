...
public:
	...
	vk::raii::DescriptorSet& GetCurrentDescriptorSet(uint32_t frameIndex) { return VulkanDescriptorSets[frameIndex]; }

	void UpdateUniformBuffer(uint32_t currentImage, const Mesh::UniformBufferObject& ubo);

	void SetUniformBuffers(std::vector<vk::raii::Buffer>& buffers, std::vector<vk::raii::DeviceMemory>& memories, std::vector<void*>& mapped);
